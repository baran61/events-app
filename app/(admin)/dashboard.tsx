...
export default function AdminScreen() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const endpoint =
        eventType === "normal" ? "/admin/events" : "/admin/upcoming";
  ...
  const pickImage = async () => {
    ...
    if (!permissionResult.granted) {
      Alert.alert(
        "İzin Gerekli",
        "Fotoğraf seçmek için izin vermeniz gerekiyor!"
      );
      return;
    }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
      ...
      {editingId && (
        ...
        >
          ...
          >
            Seçilen Tarih: {editDate.toLocaleDateString()}
          </Text>
          <Button
            title="↩️ Geri Dön"
            onPress={() => {
